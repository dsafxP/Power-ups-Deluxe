        ...
        public void HandleCommand(UserMessageCallbackArgs args) {
            ...
            switch (args.Command) {
                case "PD_HELP": {
                        ...
                        Game.ShowChatMessage("Available commands:", Color.Green, uid);
                        Game.ShowChatMessage("PD_HELP - Shows command help.", Color.Green, uid);
                ...
                case "PD_POWERUPS": {
                        ...
                        Game.ShowChatMessage("Available power-ups:", Color.Green, uid);

                        foreach (string powerUpName in typeof(Powerups.AvailablePowerups)
                                     .GetNestedTypes()
                                     .Select(t => t.Name)
                                     .OrderBy(n => n))
                            Game.ShowChatMessage(powerUpName, Color.Green, uid);
                    }
                    break;
                ...
                case "PD_CRATE_CHANCE": {
                        if (!user.IsModerator && !user.IsHost) {
                            Game.ShowChatMessage(
                                "You don't have enough perms to execute this command.", Color.Red,
                                user.UserIdentifier);
                        ...
                        if (string.IsNullOrEmpty(arg)) {
                            Game.ShowChatMessage(
                                string.Format("Special crate chance is set to {0}.",
                                              Config.SpecialCrateChance),
                        ...
                        if (float.TryParse(args.CommandArguments.Trim(), out crateChance)) {
                            ...
                            Game.ShowChatMessage(string.Format("Set special crate chance to {0}.",
                                                               Config.SpecialCrateChance),
                                                 Color.Green, user.UserIdentifier);
                        }
                        else {
                            Game.ShowChatMessage("Specify a valid number.", Color.Red,
                                                 user.UserIdentifier);
                        }
                    }
                    break;
                ...
                case "PD_SYRINGE": {
                        ...
                        IPlayer targetPlayer =
                            target != null ? target.GetPlayer() : user.GetPlayer();

                        if (targetPlayer != null) {
                            OnPowerupSyringe(new TriggerArgs(null, targetPlayer, false));
                        }
                        else {
                            Game.ShowChatMessage("Invalid player.", Color.Red, user.UserIdentifier);
                        }
                    }
                    break;
                ...
                case "PD_POWERUP": {
                        ...
                        if (powerUpType != null) {
                            ...
                            if (targetPlayer != null) {
                                Powerup powerUp =
                                    (Powerup)Activator.CreateInstance(powerUpType, targetPlayer);
                                powerUp.Enabled = true;

                                Game.ShowChatMessage(
                                    string.Format("{0} - {1}", powerUp.Name, powerUp.Author),
                                    Color.Yellow, targetPlayer.UserIdentifier);

                                PlayPowerupEffect(targetPlayer.GetWorldPosition());
                            }
                            else {
                                Game.ShowChatMessage("Invalid player.", Color.Red,
                                                     user.UserIdentifier);
                            }
                        }
                        else {
                            Game.ShowChatMessage("Invalid power-up.", Color.Red,
                                                 user.UserIdentifier);
        ...
        public static class Config {
            ...
            public static float SpecialCrateChance {
                get { return Powerups.SpawnChance; }
                set {
            ...
            public static void Update() {
                ...
                if (Game.LocalStorage.TryGetItemFloat(SPECIAL_CRATE_KEY,
                                                      out specialCrateChance)) {
        ...
        public IUser GetUser(string arg) {
            return string.IsNullOrEmpty(arg)
                       ? null
                       : Game.GetActiveUsers().FirstOrDefault(
                             u => u.AccountName == arg || u.Name == arg ||
                                  (arg.All(char.IsDigit)
                                       ? u.GameSlotIndex == int.Parse(arg)
                                       : false));
        }
        ...
        public static class Powerups {
            private static readonly ObjectAITargetData _boxTargetData =
                new ObjectAITargetData(500, ObjectAITargetMode.MeleeOnly);

            private static Events.ObjectCreatedCallback _objectCreatedCallback = null;

            public static bool Enabled {
                get { return _objectCreatedCallback != null; }
                set {
                    if (value != Enabled)
                        if (value) {
                            _objectCreatedCallback =
            ...
            public static SupplyBox CreatePowerupBox(Vector2 pos) {
                ...
                IObjectWeldJoint weldJoint =
                    (IObjectWeldJoint)Game.CreateObject("WeldJoint", pos);
                ...
                IObjectDestroyTargets destroyTargets =
                    (IObjectDestroyTargets)Game.CreateObject("DestroyTargets", pos);
            ...
            public static IObject CreatePowerupSyringe(Vector2 pos) {
                ...
                IObjectActivateTrigger activateTrigger =
                    (IObjectActivateTrigger)Game.CreateObject("ActivateTrigger", pos);
            ...
            private static void OnObjectCreated(IObject[] objs) {
                ...
                foreach (IObject supplyCrate in objs.Where(o => o.Name == "SupplyCrate00" &&
                                                                _rng.Next(101) <
                                                                    SpawnChance)) {
            ...
            public class SupplyBox {
                ...
                public bool Enabled {
                    get { return _updateCallback != null && _objTerminatedCallback != null; }
                    set {
                        if (value != Enabled)
                            if (value) {
                                _updateCallback = Events.UpdateCallback.Start(Update);
                                _objTerminatedCallback =
                    ...
                    if (_slowFall) {
                        ...
                        if (_elapsed % RAYCAST_COOLDOWN == 0) {
                            ...
                            RayCastResult result =
                                Game.RayCast(rayCastStart, rayCastEnd, _collision)[0];

                            _slowFall = !result.Hit;

                            if (!_slowFall) Box.SetLinearVelocity(Vector2.Zero);
                        }
                    }
                }
                ...
                private void OnObjectTerminated(IObject[] objs) {
                    if (objs.Any(o => o == Box)) {
                        ...
                        if (Destroyed != null) Destroyed.Invoke(Box);
                    }
                }
            }
                    ...
                    get {
                        return Game.GetObjectsByArea<IPlayer>(Trigger.GetAABB())
                            .FirstOrDefault(p => p.IsBot && !p.IsDead && p.IsInputEnabled &&
                                                 !_activators.Contains(p) &&
                                                 (Trigger.GetUseType() ==
                                                      ActivateTriggerUseType.Individual ||
                                                  !_activators.Any()));
                    }
                }
            ...
            public static class AvailablePowerups {
                ...
                public class Flame : Powerup {
                    private static readonly PlayerModifiers _fireMod =
                        new PlayerModifiers() { FireDamageTakenModifier = 0 };
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        Player.SetMaxFire();  // Ensure player has maximum fire level while
                                              // powerup is active
                    }
                    ...
                    public override void OnEnabled(bool enabled) {
                        if (!enabled) {
                            ...
                            if (Player.IsBot && _set != null) Player.SetBotBehaviorSet(_set);
                        }
                    }
                }
                ...
                public class Adrenaline : Powerup {
                    ...
                    private const float SPEED_MULT =
                        0.75f;  // Moving while punching speed multiplier
          ...
          // Keys that will trigger movement
          VirtualKey.AIM_RUN_LEFT, VirtualKey.AIM_RUN_RIGHT};

                    private bool PlayerValid {
                        get {
                            return Player.IsOnGround && !Player.IsDiving &&
                                   !Player.IsManualAiming && !Player.IsDisabled;
                        }
                    }
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        ...
                        if ((_inputKeys.Any(k => Player.KeyPressed(k)) || Player.IsBot) &&
                            ...
                            Vector2 offset = new Vector2(
                                (SPEED_MULT * Player.GetModifiers().RunSpeedModifier) *
          ...
          typeof(IObjectSupplyCrate), typeof(IObjectStreetsweeperCrate),
          typeof(IObjectWeaponItem)};
                    ...
                    private IPlayer[] PlayersInVortex {
                        get {
                            return Game.GetObjectsByArea<IPlayer>(VortexArea)
                                .Where(p => (p.GetTeam() == PlayerTeam.Independent ||
                        ...
                        if (Time % VORTEX_COOLDOWN == 0) {  // every 250ms
                            ...
                            foreach (IPlayer pulled in PlayersInVortex) {
                                ...
                                Events.UpdateCallback.Start(
                                    (float _dlt) => { pulled.SetInputEnabled(true); }, 1, 1);

                                Vector2 pulledPos = pulled.GetWorldPosition();

                                pulled.SetWorldPosition(pulledPos +
                                                        (Vector2Helper.Up * 2));  // Sticky feet

                                pulled.SetLinearVelocity(Vector2Helper.DirectionTo(
                                                             pulledPos, Player.GetWorldPosition()) *
                                                         VORTEX_FORCE);
                            ...
                            foreach (IObject pulled in ObjectsInVortex) {
                                pulled.SetLinearVelocity(
                                    Vector2Helper.DirectionTo(pulled.GetWorldPosition(),
                    ...
                    private void Draw(Vector2 pos) {
                        PointShape.Swirl((v => Game.PlayEffect(
                                              EffectName.ItemGleam,
                                              Vector2Helper.Rotated(
                                                  v - pos, (float)(Time % 1500 *
                                                                   (MathHelper.TwoPI / 1500))) +
                                                  pos)),
                                         pos,                   // Center Position
                                         5,                     // Initial Radius
                ...
                public class Sphere : Powerup {
                    ...
                    private IProjectile[] ProjectilesInSphere {
                        get {
                            return Game.GetProjectiles()
                                .Where(pr => SphereArea.Contains(pr.Position) &&
                                             pr.InitialOwnerPlayerID != Player.UniqueID &&
                                             (GetTeamOrDefault(Game.GetPlayer(
                                                  pr.InitialOwnerPlayerID)) != Player.GetTeam() ||
                    ...
                    private PlayerTeam GetTeamOrDefault(
                        IPlayer player, PlayerTeam defaultTeam = PlayerTeam.Independent) {
                ...
                public class RocketShoes : Powerup {
                    ...
                    private bool PlayerValid {
                        get {
                            return !Player.IsDisabled && !Player.IsLedgeGrabbing &&
                                   !Player.IsClimbing && !Player.IsDiving && !Player.IsGrabbing &&
                                   Player.IsInputEnabled;
                        }
                    }

                    private bool Rocketing {
                        get { return PlayerValid && Player.KeyPressed(VirtualKey.JUMP); }
                    }

                    private Vector2 Impulse {
                        get {
                            Vector2 impulse =
                                new Vector2(0, Player.GetLinearVelocity().Y + IMPULSE);

                            impulse.X +=
                                Player.KeyPressed(VirtualKey.AIM_RUN_RIGHT)
                                    ? 1
                                    : (Player.KeyPressed(VirtualKey.AIM_RUN_LEFT) ? -1 : 0);

                            impulse.X *= Player.KeyPressed(VirtualKey.SPRINT)
                                             ? 2
                    ...
                    protected override void Activate() {
                        _feet = new IObject[]{
            Game.CreateObject("InvisibleBlockNoCollision", Vector2.Zero,
                              3 / 2 * MathHelper.PI),
            Game.CreateObject("InvisibleBlockNoCollision", Vector2.Zero,
                              3 / 2 * MathHelper.PI)};
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        ...
                        foreach (IObject obj in _feet) {
                            ...
                            obj.SetAngle(Vector2Helper.AngleToPoint(
                                playerPos,
                                playerPos - new Vector2(Player.GetLinearVelocity().X,
                                                        Math.Abs(Player.GetLinearVelocity().Y))));
                        }

                        _feet[0]
                            .SetWorldPosition(playerPos + new Vector2(-5, -2));
                        _feet[1]
                            .SetWorldPosition(playerPos + new Vector2(5, -2));
                        ...
                        if (Rocketing) {
                            ...
                            if (Time % EFFECT_COOLDOWN == 0) {
                                Game.PlayEffect("MZLED", Vector2.Zero, _feet[0].UniqueID,
                                                "MuzzleFlashS");
                                Game.PlayEffect("MZLED", Vector2.Zero, _feet[1].UniqueID,
                                                "MuzzleFlashS");
                            }
                ...
                public class Clone : Powerup {
                    ...
                    protected override void Activate() {
                        ...
                        if (_clonePlayer.GetTeam() == PlayerTeam.Independent) {
                            List<PlayerTeam> AvailableTeams =
                                new List<PlayerTeam>{PlayerTeam.Team1, PlayerTeam.Team2,
                                   PlayerTeam.Team3, PlayerTeam.Team4};

                            foreach (IPlayer player in Game.GetPlayers()) {
                                if (!player.IsDead) AvailableTeams.Remove(player.GetTeam());
                            }
                        ...
                        _clonePlayer.SetCurrentMeleeDurability(
                            Player.CurrentMeleeWeapon.Durability);

                        _clonePlayer.GiveWeaponItem(
                            Player.CurrentPrimaryRangedWeapon.WeaponItem);

                        _clonePlayer.SetCurrentPrimaryWeaponAmmo(
                            Player.CurrentPrimaryRangedWeapon.CurrentAmmo,
                            Player.CurrentPrimaryRangedWeapon.SpareMags);

                        _clonePlayer.GiveWeaponItem(
                            Player.CurrentSecondaryRangedWeapon.WeaponItem);

                        _clonePlayer.SetCurrentPrimaryWeaponAmmo(
                            Player.CurrentSecondaryRangedWeapon.CurrentAmmo,
                            Player.CurrentSecondaryRangedWeapon.SpareMags);

                        _clonePlayer.GiveWeaponItem(Player.CurrentThrownItem.WeaponItem);

                        _clonePlayer.SetCurrentThrownItemAmmo(
                            Player.CurrentThrownItem.CurrentAmmo);

                        // Create a hard bot
                        _clonePlayer.SetBotBehavior(
                            new BotBehavior(true, PredefinedAIType.CompanionB));
                    ...
                    public override void OnEnabled(bool enabled) {
                        if (!enabled && _clonePlayer != null) _clonePlayer.Kill();
                    }
                }
                ...
                public class Turret : Powerup {
                    ...
                    private static readonly RayCastInput _raycastInput =
                        new RayCastInput(true) {
                            FilterOnMaskBits = true,
                    ...
                    public Turret(IPlayer player) : base(player) { Time = 17000; }
                    ...
                    private void Shoot(Vector2 target) {
                        ...
                        new CustomProjectile(
                            _wisp.Position, Vector2Helper.DirectionTo(_wisp.Position, target),
                            _raycastInput) {
                    ...
                    private void _OnPlayerHit(IPlayer hit, Vector2 pos) {
                        ...
                        hit.DealDamage(DMG * hit.GetModifiers().ProjectileDamageTakenModifier);
                    ...
                    public override void OnEnabled(bool enabled) {
                        if (_wisp != null) _wisp.Enabled = enabled;
                    }

                    private class Wisp {
                        private const uint EFFECT_COOLDOWN = 50;

                        private static readonly RayCastInput _raycastInput = new RayCastInput(
                            true) {
                            IncludeOverlap = true,
                            FilterOnMaskBits = true,
                            MaskBits = ushort.MaxValue,
                            ...
                            get {
                                ...
                                enemies.Sort((p1, p2) =>
                                                 Vector2.Distance(p1.GetWorldPosition(), playerPos)
                                                     .CompareTo(Vector2.Distance(
                                                         p2.GetWorldPosition(), playerPos)));
                        ...
                        public Vector2 Position {
                            get { return Player.GetWorldPosition() + Offset; }
                        }
                            ...
                            if (OnShoot != null && CanFire) {
                                ...
                                if (closestPlayer != null) {
                                    ...
                                    RayCastResult rayCastResult =
                                        Game.RayCast(position, closestTarget, _raycastInput)[0];

                                    if (rayCastResult.IsPlayer) OnShoot.Invoke(closestTarget);
                                }
                            }
                        }
                    }
                ...
                public class StoneSkin : Powerup {
                    ...
                    private IProfile StoneProfile {
                        get {
                            ...
                            return ColorProfile(playerProfile, "ClothingGray",
                                                "ClothingLightGray");
                        }
                    }
                    ...
                    private static IProfile ColorProfile(IProfile pr, string col,
                                                         string colI) {
                        if (pr.Accesory != null)
                        ...
                        if (pr.ChestUnder != null)
                            pr.ChestUnder =
                                new IProfileClothingItem(pr.ChestUnder.Name, col, colI);
                ...
                public class FireBreath : Powerup {
                    ...
                    private static readonly Type[] _types = { typeof(IPlayer) };

                    private static readonly RayCastInput _rayCastInput =
                        new RayCastInput(true) { Types = _types };
                    ...
                    private bool EnemiesInRange {
                        get {
                            ...
                            if (result.IsPlayer) {
                                ...
                                return (hit.GetTeam() == PlayerTeam.Independent ||
                                        hit.GetTeam() != Player.GetTeam()) &&
                                       !hit.IsDead;
                            }
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        ...
                        if (Time % EFFECT_COOLDOWN == 0)  // Effect
                            Game.PlayEffect(EffectName.FireTrail,
                                            Player.GetWorldPosition() + _effectOffset);
                        ...
                        if (Time % FIRE_RATE == 0)  // Attack
                            if (EnemiesInRange) {
                                ...
                                Game.SpawnFireNode(Player.GetWorldPosition() + fireOffset,
                                                   GetRandomFireVelocity(_rng) * SPEED, FIRE_TYPE);
                            }
                    }
                    ...
                    private Vector2 GetRandomFireVelocity(Random random) {
                        float x = (_rayCastEndOffset.X * Player.FacingDirection) *
                                  (float)(RANDOM_SPEED_EXP * random.NextDouble());
                        float y = _rayCastEndOffset.Y *
                                  (float)(RANDOM_SPEED_EXP * random.NextDouble());
                ...
                public class ManaShield : Powerup {
                    ...
                    private readonly Events.CallbackDelegate[] handlers =
                        new Events.CallbackDelegate[2];
                    ...
                    public ManaShield(IPlayer player) : base(player) { Time = MAX_TIME; }
                    ...
                    protected override void Activate() {
                        ...
                        IObjectWeldJoint weld1 = (IObjectWeldJoint)Game.CreateObject(
                            "WeldJoint", Player.GetWorldPosition() +
                                             offset);  // Direct attachment to player by center1
                        IObjectWeldJoint weld2 = (IObjectWeldJoint)Game.CreateObject(
                            "WeldJoint",
                            Player.GetWorldPosition() +
                                offset);  // Rotating attachments around center1, by center2
                        IObjectWeldJoint weld3 = (IObjectWeldJoint)Game.CreateObject(
                            "WeldJoint",
                            Player.GetWorldPosition() +
                                offset);  // attached to player by proxy through center1

                        allItems.Add(weld1);
                        allItems.Add(weld2);
                        allItems.Add(weld3);

                        IObject center1 = (IObject)Game.CreateObject(
                            centerobj, Player.GetWorldPosition() +
                                           offset);  // HINGE FOR ROTATING PART TO ATTACH TO,
                                                     // WELDED ONTO PLAYER
                        ...
                        IObjectPullJoint force = (IObjectPullJoint)Game.CreateObject(
                            "PullJoint", center1.GetWorldPosition() + new Vector2(0, 200));
                        // force.SetLineVisual(LineVisual.DJRope);
                        force.SetForcePerDistance(0.01f);
                        allItems.Add(force);

                        bird = Game.CreateObject(
                            centerobj, center1.GetWorldPosition() + new Vector2(0, 200));
                        force.SetTargetObject(bird);
                        allItems.Add(bird);

                        IObjectTargetObjectJoint target =
                            (IObjectTargetObjectJoint)Game.CreateObject(
                                "TargetObjectJoint", center1.GetWorldPosition());
                        target.SetTargetObject(center1);
                        force.SetTargetObjectJoint(target);
                        allItems.Add(target);

                        IObject center2 = (IObject)Game.CreateObject(
                            centerobj, Player.GetWorldPosition() + offset);
                        ...
                        IObjectRevoluteJoint revolute =
                            (IObjectRevoluteJoint)Game.CreateObject(
                                "RevoluteJoint", Player.GetWorldPosition() + offset);
                        ...
                        for (int i = 0; i < 4; i++) {
                            IObjectText obj = (IObjectText)Game.CreateObject(
                                "Text", center1.GetWorldPosition() +
                                            Vector2Helper.Rotated(new Vector2(-22, 2),
                        ...
                        IObject deflector = Game.CreateObject(
                            "InvisibleBlockNoCollision",
                            Player.GetWorldPosition() + new Vector2(-17, 2.3f));
                        ...
                        deflector.SetSizeFactor(new Point(
                            4, 4));  // setmass doesnt come into effect if called too early
                        deflector.SetMass(0.000001f);
                        weld2.AddTargetObject(deflector);
                        allItems.Add(deflector);

                        IObjectText shine = (IObjectText)Game.CreateObject(
                            "Text", new Vector2(-5, -1) + center1.GetWorldPosition());
                        ...
                        IObjectText crack1 = (IObjectText)Game.CreateObject(
                            "Text", center1.GetWorldPosition() + new Vector2(-8.6f, 14.5f));
                        ...
                        IObjectText crack2 = (IObjectText)Game.CreateObject(
                            "Text", center1.GetWorldPosition() + new Vector2(12.8f, 6.9f));
                        ...
                        IObjectText crack3 = (IObjectText)Game.CreateObject(
                            "Text", center1.GetWorldPosition() + new Vector2(6f, -5.3f));
                        ...
                        onHit = Events.ProjectileHitCallback.Start((projectile, args) => {
                            if (Game.GetObject(args.HitObjectID).CustomID ==
                                "deflector") {  // remove getobject
                                Vector2 normal = Vector2Helper.Rotated(
                                    Vector2.Normalize(projectile.Position -
                                                      center1.GetWorldPosition()),
                                    MathHelper.PI);

                                double angleDifference = Math.Abs(Vector2Helper.AngleTo(
                                    normal,
                                    projectile
                                        .Velocity));  // Math.Abs(Vector2Helper.Angle(normal) -
                                                      // Vector2Helper.Angle(projectile.Velocity));

                                if (angleDifference < MathHelper.PIOver2) {
                                    if (projectile.ProjectileItem ==
                                            ProjectileItem.GRENADE_LAUNCHER ||
                                        projectile.ProjectileItem == ProjectileItem.BAZOOKA ||
                                        projectile.ProjectileItem == ProjectileItem.FLAKCANNON) {
                                    ...
                                    projectile.Velocity =
                                        Vector2Helper.Bounce(projectile.Velocity, normal);
                                    projectile.Position += normal * 2;

                                    health -= (projectile.GetProperties().ObjectDamage *
                                               (float)(angleDifference / MathHelper.PI)) +
                                              (projectile.GetProperties().ObjectDamage) / 3;
                        ...
                        onDamage = Events.PlayerDamageCallback.Start(
                            (IPlayer hitPlayer, PlayerDamageArgs args) => {
                                ...
                                if (hitPlayer.UniqueID == Player.UniqueID) {
                                    ...
                                    if (modhp.CurrentHealth == 0)
                                        ...
                                        modhp.CurrentHealth +=
                            args.Damage;  // THIS DOESNT BLOCK ALL DAMAGE
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        ...
                        if (Time % 50 == 0) {
                            if (_rng.Next(0, 6) == 1) {
                                Game.PlayEffect(
                                    EffectName.ItemGleam,
                                    RandomPoint(RADIUS - 6) + Player.GetWorldPosition() + offset);
                            }
                        }
                    ...
                    public void TurnEffect() {
                        for (int i = 0; i < effects.Length; i++) {
                            effects[i]
                                .SetText(
                                    effects[i].CustomID);  // setting to effect at the last sec
                        }
                    }
                    ...
                    private void BreakShield() {
                        ...
                        for (int i = 0; i < 50; i++) {
                            ...
                            if (_rng.Next(0, 2) == 0) {
                                IObject debris = Game.CreateObject(
                                    "GlassShard00A", Player.GetWorldPosition() +
                                                         new Vector2(X_OFFSET, Y_OFFSET) + dir);
                                debris.SetHealth(1);
                                ...
                                Game.PlayEffect(EffectName.DestroyGlass,
                        ...
                        cleanUp = Events.UpdateCallback.Start(_dlt => {
                            if (toFade.Count() > 0) {
                                Game.PlaySound("GlassShard",
                                               toFade[toFade.Count() - 1]
                                                   .GetWorldPosition(),
                                               5);
                                toFade[toFade.Count() - 1]
                                    .Remove();
                ...
                public class AirDash : Powerup {
                    ...
                    private void OnEmptyUppercut() {
                        ...
                        Game.PlayEffect(EffectName.TraceSpawner, Vector2.Zero, Player.UniqueID,
                                        EffectName.DustTrail, 1.5f);

                        Player.SetWorldPosition(Player.GetWorldPosition() +
                                                Vector2Helper.Up * 2);  // Sticky feet
                    ...
                    private void EmptyUppercutCheck(float dlt) {
                        ...
                        bool[] checks = {
            Player.IsMeleeAttacking, playerXVelocityAbs >= 0.4f,
            ...
            // Player.CurrentWeaponDrawn == WeaponItemType.NONE
                ...
                public class Thorns : Powerup {
                    ...
                    private void OnPlayerMeleeAction(IPlayer attacker,
                                                     PlayerMeleeHitArg[] args) {
                        foreach (PlayerMeleeHitArg arg in args.Where(a => a.HitObject ==
                                                                          Player)) {
              ...
              WeaponItemType.Melee, WeaponItemType.Rifle,
              WeaponItemType.Handgun, WeaponItemType.Powerup,
              WeaponItemType.Thrown};
                ...
                public class Blitzkrieg : Powerup {
                    ...
                    private IObject Throw(bool missile = false) {
                        ...
                        Vector2 throwVel = Vector2Helper.DirectionTo(
                                               playerPos, closestEnemy.GetWorldPosition()) *
                                           THROW_SPEED;
          ...
          PlayerDamageEventType.Missile};
                ...
                public class Berserk : Powerup {
                    ...
                    private static readonly PlayerModifiers _berserkMod =
                        new PlayerModifiers() {
                            CurrentHealth = 1,
                            ProjectileCritChanceTakenModifier = 0,
                            ExplosionDamageTakenModifier = 0,
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        if (Player.KeyPressed(VirtualKey.JUMP) && !Player.IsInMidAir &&
                            !Player.IsDisabled) {
                ...
                public class Kamikaze : Powerup {
                    ...
                    private const string TXT_EFFECT =
                        "EXPLODING IN {0}...";  // 0 for current tick

                    private static readonly Vector2[] _explosionsOffset = {
          // +
          Vector2.Zero, new Vector2(40, 0), new Vector2(-40, 0),
          new Vector2(0, 40), new Vector2(0, -40)};

                    private static readonly PlayerModifiers _explodeMod =
                        new PlayerModifiers() { ExplosionDamageTakenModifier = 0.125f };
                ...
                public class Punchback : Powerup {
                    private const string TXT_EFFECT =
                        "BULLETS LEFT: {0}";  // 0 for bullets left
                    ...
                    private void OnPlayerMeleeAction(IPlayer player,
                                                     PlayerMeleeHitArg[] args) {
                        if (player != Player) return;
                        ...
                        if (_bulletsAbsorbed > 0 && closestEnemy != null) {
                            ...
                            Game.SpawnProjectile(PROJ_ITEM, bulletPos,
                                                 Vector2Helper.DirectionTo(
                                                     bulletPos, closestEnemy.GetWorldPosition()),
                                                 PROJ_POWERUP);
                        }
                    }

                    private void OnProjectileHit(IProjectile projectile,
                                                 ProjectileHitArgs args) {
          ...
          "WpnGrenadesThrown", "WpnMolotovsThrown", "WpnC4Thrown",
          "WpnMineThrown", "WpnShurikenThrown"};
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        ...
                        foreach (IObject affected in ObjectsInMagnet) {
                            affected.SetLinearVelocity(
                                Vector2Helper.DirectionTo(affected.GetWorldPosition(),
                                                          Player.GetWorldPosition()) *
                                MAGNET_FORCE);
                        ...
                        foreach (IObject explosives in ExplosivesInMagnet) {
                            explosives.SetLinearVelocity(
                                Vector2Helper.DirectionTo(Player.GetWorldPosition(),
                                                          explosives.GetWorldPosition()) *
                                MAGNET_FORCE);

                            explosives.SetAngularVelocity(MAGNET_ANGULAR_SPEED);

                            if (effect)
                                Game.PlayEffect(EffectName.ItemGleam,
                                                explosives.GetWorldPosition());
                        }

                        foreach (IObjectStreetsweeper sweepers in
                                     TargetStreetsweepersInMagnet) {
                        ...
                        if (effect) {
                            PointShape.Circle(Draw, Player.GetWorldPosition(), MAGNET_RADIUS,
                                              EFFECT_SEPARATION);
                        }
                    }
                ...
                public class Telekinesis : Powerup {
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        if (_sticky != null) {
                            ...
                            _sticky.SetLinearVelocity(Vector2Helper.DirectionTo(stickyPos, pos) *
                                                      VORTEX_FORCE);
                        ...
                        foreach (IProjectile fired in Game.GetProjectiles().Where(
                                     p => p.OwnerPlayerID == Player.UniqueID)) {
                            fired.Direction = inputDirection;
                            fired.Velocity = inputDirection != Vector2.Zero ? inputDirection *
                                                                   BULLET_FORCE
                    ...
                    private void OnPlayerKeyInput(IPlayer player,
                                                  VirtualKeyInfo[] keyEvents) {
                        if (player != Player) return;

                        foreach (VirtualKeyInfo pressed in keyEvents.Where(
                                     k => k.Event == VirtualKeyEvent.Pressed &&
                                          k.Key == VirtualKey.ACTIVATE)) {
                            if (_sticky != null) {
                                _sticky.TrackAsMissile(true);

                                IPlayer closestEnemy = ClosestEnemy;

                                _sticky.SetLinearVelocity(closestEnemy !=
                                                              null ? Vector2Helper.DirectionTo(
                                                                  _sticky.GetWorldPosition(),
                                                                  closestEnemy.GetWorldPosition()) *
                                                                  LAUNCH_FORCE
                                                          : Vector2.Zero);
                    ...
                    private void OnObjectCreated(IObject[] objs) {
                        _thrown.AddRange(objs.Where(
                            o => o.IsMissile && TrackThrowArea.Intersects(o.GetAABB())));
                    }
                }

                // ARSENAL - dsafxP
                public class Arsenal : Powerup {
                    private static readonly PlayerModifiers _arsenalMod =
                        new PlayerModifiers() { ItemDropMode = 1 };
                    ...
                    private static WeaponItem RandomWeapon {
                        get {
                            ...
                            while (w == WeaponItem.STREETSWEEPER) w = Game.GetRandomWeaponItem();
              ...
              WeaponItemType.Thrown};
                    ...
                    protected override void Activate() {
                        ...
                        if (ground != Vector2.Zero) {
                            ...
                            _stash = (IObjectAmmoStashTrigger)
                                         Game.CreateObject("AmmoStash00", ground);
                        }
                    }
                    ...
                    private static WeaponItem GetRandomWeaponFromType(WeaponItemType type) {
                        ...
                        IObjectWeaponItem wItem =
                            Game.SpawnWeaponItem(w, Vector2.Zero, false, float.Epsilon);

                        while (wItem.WeaponItemType != type) {
                            w = RandomWeapon;

                            wItem = Game.SpawnWeaponItem(w, Vector2.Zero, false, float.Epsilon);
                        }
                ...
                public class Giant : Powerup {
                    ...
                    private static readonly PlayerModifiers _giantMod =
                        new PlayerModifiers() {
                        ...
                        get {
                            return SHAKE_INTENSITY * (Player.IsRunning ? 1.5f
                                                      : (Player.IsSprinting ? 2
                                                         : (Player.IsLayingOnGround ||
                                                            Player.IsRecoveryKneeling ? 3 : 1)));
                        }
                    }
                    ...
                    private IObject[] ObjectsShaking {
                        get {
                            return Game.GetObjectsByArea(ShakeArea)
                                .Where(o => !(o is IPlayer) &&
                                            o.GetBodyType() == BodyType.Dynamic && o.Destructable)
                                .ToArray();
                        }
                    }
                    ...
                    public Giant(IPlayer player) : base(player) { Time = TIME; }
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        ...
                        if (Player.IsBurningInferno) Player.ClearFire();

                        if (Player.IsBoostHealthActive) Player.Kill();
                        ...
                        if (shake != SHAKE_INTENSITY) {
                            if (Time % EFFECT_COOLDOWN == 0)
                                Game.PlayEffect(EffectName.CameraShaker, Vector2.Zero,
                                                shake / SHAKE_EFFECT_DAMP, EFFECT_COOLDOWN * 2,
                                                true);
                            ...
                            if (CanShake) {
                                ...
                                foreach (IPlayer shaking in PlayersShaking) {
                                    ...
                                    Events.UpdateCallback.Start(
                                        (float _dlt) => { shaking.SetInputEnabled(true); }, 1, 1);

                                    shaking.SetWorldPosition(shaking.GetWorldPosition() +
                                                             (Vector2Helper.Up * 2));  // Sticky feet
                                ...
                                foreach (IObject shaking in ObjectsShaking) {
                                    shaking.SetLinearVelocity(shaking.GetLinearVelocity() +
                                                              shakeVect);
                                    ...
                                    Game.PlayEffect(EffectName.DestroyDefault,
                                                    shaking.GetWorldPosition());
                                }
                            }
                        }
                    }
                    ...
                    private void OnPlayerMeleeAction(IPlayer player,
                        ...
                        foreach (PlayerMeleeHitArg arg in args.Where(a => !a.IsPlayer)) {
                            IObject hit = arg.HitObject;
                            float dmg = arg.HitDamage;

                            hit.SetHealth(hit.GetHealth() +
                                          dmg);  // Null dmg so it isn't dealt twice
                ...
                public class Drone : Powerup {
                    ...
                    protected override void Activate() {
                        Streetsweeper = (IObjectStreetsweeper)Game.CreateObject(
                            "Streetsweeper", Player.GetWorldPosition());
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        ...
                        float angle = enemy != null ? Vector2Helper.AngleToPoint(
                                                   Streetsweeper.GetWorldPosition(),
                ...
                public class Overcharge : Powerup {
                    ...
                    private int Charges {
                        ...
                        set {
                            ...
                            Player.SetModifiers(
                                new PlayerModifiers() { MeleeForceModifier = Math.Max(charge, 1) });
                        }
                    }
                    ...
                    public override string Author {
                        get { return "dsafxP - Danila015 - Eiga"; }
                    }
                    ...
                    private void OnPlayerMeleeAction(IPlayer player,
                        ...
                        IEnumerable<IPlayer> stunned =
                            args.Where(a => a.IsPlayer)
                                .Select(p => (IPlayer)p.HitObject)
                                .Where(p => (p.IsFalling || p.IsStaggering) && !p.IsDead);

                        if (stunned.Any()) Charges += stunned.Count();
                    }
                }
                ...
                public class Vampirism : Powerup {
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        if (Time % EFFECT_COOLDOWN == 0)
                            Game.PlayEffect(EffectName.BloodTrail,
                                            Player.GetWorldPosition() + _offset);
                    ...
                    private void OnPlayerMeleeAction(IPlayer player,
                        ...
                        foreach (PlayerMeleeHitArg arg in args.Where(a => a.IsPlayer)) {
                            ...
                            if (!_bleeding.Contains(hit)) {
                                ...
                                Events.UpdateCallback.Start(
                                    (float _dlt) => { _bleeding.Remove(hit); }, BLEED_TIME, 1);
                            }
                        }
                    }
                }
          ...
          new Vector2(-10, 0), new Vector2(10, 0), new Vector2(0, 20)};

                    private Events.PlayerMeleeActionCallback _meleeActionCallback = null;

                    private Vector2[] EffectPositions {
                        get {
                            return _offsets.Select(o => o += Player.GetWorldPosition()).ToArray();
                        }
                    }

                    public override string Name {
                        get { return "TRI-STRIKE"; }
                    }

                    public override string Author {
                        get { return "dsafxP - Eiga"; }
                    }

                    public Strike(IPlayer player) : base(player) { Time = 17000; }
                    ...
                    private void OnPlayerMeleeAction(IPlayer player,
                        ...
                        foreach (PlayerMeleeHitArg arg in args) {
                            ...
                            Events.UpdateCallback.Start(
                                (float _dlt) => { hit.SetInputEnabled(true); }, 1, 1);
                            ...
                            if (!hit.IsBlocking) {
                                ...
                                hit.SetWorldPosition(hit.GetWorldPosition() +
                                                     (Vector2Helper.Up * 2));  // Sticky feet

                                hit.SetLinearVelocity(
                                    hit.GetLinearVelocity() +
          ...
          WeaponItemType.Handgun, WeaponItemType.Rifle};
                    ...
                    private Events.ProjectileCreatedCallback _projectileCreatedCallback =
                        null;
                    private Events.PlayerWeaponAddedActionCallback _weaponAddedCallback =
                        null;
                    ...
                    protected override void Activate() {
                        if (HasRangedWeapon) Player.GiveWeaponItem(WeaponItem.LAZER);
                    }
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        if (Time % AMMO_REGEN_COOLDOWN == 0 && HasRangedWeapon) {
                            ...
                            Player.SetCurrentPrimaryWeaponAmmo(
                                (int)(rifleWeaponItem.TotalAmmo +
                                      (rifleWeaponItem.MaxTotalAmmo * AMMO_REGEN)));

                            HandgunWeaponItem handgunWeaponItem = Player.CurrentSecondaryWeapon;

                            Player.SetCurrentSecondaryWeaponAmmo(
                                (int)(handgunWeaponItem.TotalAmmo +
                                      (handgunWeaponItem.MaxTotalAmmo * AMMO_REGEN)));
                    ...
                    public override void OnEnabled(bool enabled) {
                        if (enabled) {
                            ...
                            _weaponAddedCallback = Events.PlayerWeaponAddedActionCallback.Start(
                                OnPlayerWeaponAddedAction);
                            _projectileHitCallback =
                                Events.ProjectileHitCallback.Start(OnProjectileHit);
                        }
                        else {
                            _objectCreatedCallback.Stop();
                    ...
                    private void OnObjectCreated(IObject[] objs) {
                        foreach (IObject obj in objs) {
                            if (!obj.IsMissile && !TrackThrowArea.Intersects(obj.GetAABB()))
                                continue;

                            obj.SetLinearVelocity(obj.GetLinearVelocity() * THROW_VEL_MULT);
                            obj.SetAngularVelocity(obj.GetAngularVelocity() * THROW_VEL_MULT);

                            Game.PlayEffect(EffectName.TraceSpawner, Vector2.Zero, obj.UniqueID,
                                            EffectName.ItemGleam, 2f);
                        }
                    }
                    ...
                    private void OnProjectileCreated(IProjectile[] projectiles) {
                        foreach (IProjectile proj in projectiles) {
                            if (proj.OwnerPlayerID != Player.UniqueID) continue;

                            proj.Direction = Player.AimVector;
                        }
                    }

                    private void OnPlayerWeaponAddedAction(IPlayer player,
                                                           PlayerWeaponAddedArg arg) {
                        if (player != Player) return;
                    ...
                    private void OnProjectileHit(IProjectile projectile,
                                                 ProjectileHitArgs args) {
                        if (projectile.OwnerPlayerID != Player.UniqueID) return;
          ...
          "0",         "null",          "NaN",       "N/A",
          "undefined", "nullReference", "exception", "error",
          "unknown",   "none",          "missing",   "invalid",
          "failure",   "0x0",           "false",     "empty",
          "nil",       "void"};
                    ...
                    private float PingFactor {
                        get {
                            float p = Player.GetUser().Ping * PING_INTENSITY;
                ...
                public class Reconstructor : Powerup {
                    ...
                    private static readonly string[] _debrisNames = { "Debris", "Giblet" };
                    ...
                    private IProfile MetalProfile {
                        get {
                            ...
                            return ColorProfile(playerProfile, "ClothingLightGray",
                                                "ClothingLightGray");
                        }
                    }
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        ...
                        foreach (IObject deb in DebrisInMagnet) {
                            ...
                            deb.SetLinearVelocity(
                                Vector2Helper.DirectionTo(debPos, Player.GetWorldPosition()) *
                                MAGNET_FORCE);

                            deb.SetAngularVelocity(MAGNET_ANGULAR_SPEED);

                            if (effect) Game.PlayEffect(EffectName.ItemGleam, debPos);
                        }
                    }
                ...
                public class Burst : Powerup {
                    ...
                    private readonly List<BParticle> _followingParticles =
                        new List<BParticle>();
                    ...
                    public void OnPlayerKeyInput(IPlayer player, VirtualKeyInfo[] keyEvents) {
                        if (player.UserIdentifier == Player.UserIdentifier &&
                            ...
                            Game.PlayEffect("CFTXT",
                                            Player.GetWorldPosition() + new Vector2(0, 5),
                            ...
                            finder = Events.PlayerMeleeActionCallback.Start(
                                (IPlayer attacker, PlayerMeleeHitArg[] args) => {
                                  ...
                                  new Vector2(5 * Player.FacingDirection * damageMult,
                                              6));
                                                if (arg.HitDamage <= 0) {
                    ...
                    private class BParticle {
                        ...
                        public static BParticle GetBParticle(IPlayer following, Vector2 start,
                            ...
                            particle.path2.SetWorldPosition(following.GetWorldPosition() +
                                                            offset2 +
                        ...
                        private BParticle() {
                            ...
                            // the rails
                            path2 =
                                (IObjectElevatorPathJoint)Game.CreateObject("ElevatorPathJoint");
                        ...
                        public void Update() {
                            ...
                            float distanceToTarget = Vector2.Distance(path1.GetWorldPosition(),
                        ...
                        public bool GetActive() {
                            return graphic.GetText() != string.Empty ||
                                   graphic.GetTextScale() < 0.1f;
                        }
                ...
                public class Sentry : Powerup {
                    ...
                    private static readonly CollisionFilter _barrelCollision =
                        new CollisionFilter() { CategoryBits = 0, MaskBits = 0, AboveBits = 0 };
                    ...
                    protected override void Activate() {
                        ...
                        _revoluteJoint = (IObjectRevoluteJoint)
                                             _sentryObjs.First(o => o.Name == "RevoluteJoint");
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        ...
                        if (CanFire) {
                            ...
                            Vector2 end = revoluteJointPos +
                                          (Vector2Helper.FromAngle(
                                               _revoluteJoint.GetTargetObjectB().GetAngle() -
                                               MathHelper.PIOver2) *
                                           RANGE);
                            ...
                            RayCastResult rayCastResult =
                                Game.RayCast(revoluteJointPos, end, _raycastInput)[0];

                            if (rayCastResult.IsPlayer) {
                                IPlayer hit = (IPlayer)rayCastResult.HitObject;

                                if ((hit.GetTeam() != Player.GetTeam() ||
                                     hit.GetTeam() == PlayerTeam.Independent) &&
                                    !hit.IsDead && hit != Player) {
                                    _elapsed = COOLDOWN;
                    ...
                    public override void OnEnabled(bool enabled) {
                        if (!enabled) {
                            Vector2 explosionPos =
                                _sentryObjs.First(s => s != null && !s.IsRemoved)
                                    .GetWorldPosition();

                            Game.TriggerExplosion(explosionPos);

                            foreach (IObject obj in _sentryObjs) obj.Destroy();
                        }
                    }
                    ...
                    private void Shoot(Vector2 target) {
                        ...
                        new CustomProjectile(
                            revoluteJointPos,
                            Vector2Helper.DirectionTo(revoluteJointPos, target),
                            _raycastInput) {
                    ...
                    private void _OnObjectHit(IObject hit, Vector2 pos) {
                        if (_sentryObjs.Any(o => o == hit)) return;
                    ...
                    private static IObject[] SpawnTurret(Vector2 pos) {
                        ...
                        IObjectWeldJoint weld2 =
                            (IObjectWeldJoint)Game.CreateObject("WeldJoint", pos);

                        IObjectRevoluteJoint revoluteJoint =
                            (IObjectRevoluteJoint)Game.CreateObject("RevoluteJoint", pos);

                        IObjectAlterCollisionTile alt2 =
                            (IObjectAlterCollisionTile)Game.CreateObject("AlterCollisionTile");

                        alt2.SetDisablePlayerMelee(true);
                        alt2.SetDisableProjectileHit(true);

                        IObject rail = Game.CreateObject(
                            "MetalRailing00", pos + new Vector2(8, 0), MathHelper.PIOver2);
                        ...
                        IObject hatch =
                            Game.CreateObject("MetalHatch00A", pos + new Vector2(18, 0));
                        ...
                        return new IObject[]{alt1,  weld1,         metal, metal2, pulley,
                             weld2, revoluteJoint, alt2,  rail,   hatch};
                    }
                }

                // CHANGE: "Hungry" namespace and name
                // HUNGRY - Danger Ross
          ...
          VirtualKey.AIM_RUN_LEFT, VirtualKey.AIM_RUN_RIGHT};
                    ...
                    protected override void Activate() {
                        ...
                        IObjectTargetObjectJoint sConnection =
                            (IObjectTargetObjectJoint)Game.CreateObject(
                                "TargetObjectJoint", _skull.GetWorldPosition());
                    ...
                    private void SpawnCell() {
                        for (int i = 0; i < MAX_CELLS; i++) {
                            if (_cells[i] == null || _cells[i].IsRemoved) {
                                ...
                                Vector2 pos =
                                    _body.GetWorldPosition() + randPos - new Vector2(0, -8);
                    ...
                    public override void Update(float dlt, float dltSecs) {
                        // Implement in derived classes
                        Vector2 newPos =
                            _body.GetWorldPosition() + new Vector2(0, FORCE_DISTANCE);
                        ...
                        if (_jumpCooldown > 0) {
                            ...
                            if (TouchingGround()) {
                                ...
                                newPos += new Vector2(
                                    0, ((float)Math.Floor(
                                            Math.Pow(_jumpCooldown - JUMP_COOLDOWN, 2) / 500) -
                                        1500f) /
                                           -50);
                            }
                        }
                        ...
                        int facingDirection =
                            Player.KeyPressed(VirtualKey.AIM_RUN_RIGHT) ? 1 : -1;

                        if (_inputKeys.Any(k => Player.KeyPressed(k))) {
                            // Calculate offset
                            if (TouchingGround()) {
                                newPos +=
                                    new Vector2((13f * Player.GetModifiers().RunSpeedModifier) *
                                                    facingDirection,
                                                FORCE_DISTANCE / 20f);
                            }
                            else {
                                newPos +=
                                    new Vector2((10f * Player.GetModifiers().RunSpeedModifier) *
                                                    facingDirection,
                                                0f);
                            }
                        }
                    ...
                    private class Tendril {
                        ...
                        public Tendril(Vector2 velocity, IObject body) {
                            ...
                            _forceSolid =
                                Game.CreateObject("InvisibleBlockNoCollision", forcePos);
                        ...
                        private void CheckGrab() {
                            if (Math.Floor(_grabber.GetAngularVelocity() * 1000) != 0f ||
                                ...
                                RayCastResult result = new RayCastResult(
                                    false, 0, null, false, 0, Vector2.Zero, Vector2.Zero);
                                for (int i = 0; i < 4; i++) {
                                    result = Game.RayCast(
                                        pos,
                                        pos + Vector2Helper.Rotated(
                                                  new Vector2(
                                                      5f * (_grabber.GetFaceDirection() < 0 ? -1 : 1),
                                                      0),
                                                  (float)Math.PI / 2 * i),
                                        input)[0];
                                    ...
                                    if (result.Hit) {
                                        if (result.HitObject.Name.Substring(0, 3) == "Gib") {
                                            result = new RayCastResult(false, 0, null, false, 0,
                                ...
                                if (result.Hit &&
                                    result.ObjectID != _arm.GetTargetObject().UniqueID) {
                ...
                set {
                    if (value != Enabled) {
                        if (value) {
                            ...
                            Activate();  // ADDITION FOR LESS REDUNDANCE
        ...
        public static class PointShape {
            ...
            /// <param name="pointDistance">The distance between each point on the
            /// square.</param>
            public static void Square(Action<Vector2> func, Area area,
                                      float pointDistance = 0.1f) {
            ...
            /// <param name="pointDistance">The distance between each point on the
            /// polygon.</param>
            public static void Polygon(Action<Vector2> func, Vector2[] points,
                                       float pointDistance = 0.1f) {
            ...
            public static void Swirl(Action<Vector2> func, Vector2 centerPoint,
                                     ...
                                     int revolutions = 1, int pointsPerRevolution = 360) {
            ...
            public static void Wave(Action<Vector2> func, Vector2 start, Vector2 end,
                                    float amplitude = 1, float frequency = 1,
                                    float pointDistance = 0.1f) {
        ...
        public static class Vector2Helper {
            ...
            public static Vector2 MoveToward(Vector2 v, Vector2 to, float delta) {
                ...
                if (len <= delta || len < float.Epsilon) return to;
        ...
        /// <summary>
        /// Represents a custom projectile with customizable behavior and collision
        /// handling.
        /// </summary>
        public class CustomProjectile {
            ...
            /// <param name="hitPosition">The position at which the projectile
            /// hit.</param>
            ...
            /// <param name="rayCastCollision">Information about the collision to be used
            /// for ray casting.</param>
            public CustomProjectile(Vector2 pos, Vector2 direction,
                                    RayCastInput rayCastCollision) {
                Position = pos;
                Direction = direction;
            ...
            private void Update(float dlt) {
                ...
                RayCastResult checkedResult =
                    Game.RayCast(_subPosition, Position, RayCastCollision)[0];
                ...
                if (checkedResult.Hit) {
                    if (checkedResult.IsPlayer && OnPlayerHit != null) {
                        ...
                        if (!dodged) OnPlayerHit.Invoke(hitPlayer, checkedResult.Position);
                    }
                    ...
                    Enabled =
                        !(!Piercing && !dodged ||
                          !checkedResult.IsPlayer && !checkedResult.HitObject.Destructable);
                }

                Game.PlayEffect(Effect, _subPosition);

                Vector2 trailEnd = checkedResult.Hit && !dodged &&
                                   !Piercing ? checkedResult.Position : Position;
            ...
            private static void Trail(Action<Vector2> func, Vector2 start, Vector2 end,
                                      float pointDistance = 0.1f) {
                int count =
                    (int)Math.Ceiling(Vector2.Distance(start, end) / pointDistance);
