...
public class Burst : Powerup {
  ...
  public void OnPlayerKeyInput(IPlayer player, VirtualKeyInfo[] keyEvents) {
    if (player.UserIdentifier == Player.UserIdentifier && keyEvents[0].Key == VirtualKey.ATTACK &&
      ...
      Game.PlayEffect("CFTXT", Player.GetWorldPosition() + new Vector2(0, 5), "x" + ((int)(damageMult * 100) / 100 f));
      ...
      Game.PlayEffect("CAM_S", Player.GetWorldPosition(), 2 f, 500 f, false);
  ...
  public override void Update(float dlt, float dltSecs) {
    ...
    if (_nextParticleTime <= 0) { //replace with while
      _followingParticles.Add(BParticle.GetBParticle(Player, Player.GetWorldPosition() +
        ...
        ((float)(_rng.NextDouble() * 30 f) + 10 f), ((float) _rng.NextDouble() * 4 f + 2)));
  ...
  private class BParticle {
    ...
    public static BParticle GetBParticle(IPlayer following, Vector2 start, float size) {
      ...
      Vector2 offset1 = new Vector2(-8 f, 8 f);
      ...
      particle.path2.SetWorldPosition(following.GetWorldPosition() + offset2 + following.GetLinearVelocity() * 5 f);
    ...
    public void Update() {
      ...
      float distanceToTarget = Vector2.Distance(path1.GetWorldPosition(), path2.GetWorldPosition()) - 3 f;
      float distanceToTravel = Vector2.Distance(graphic.GetWorldPosition(), path2.GetWorldPosition()) - 3 f;

      if (distanceToTravel < 3 f) {
        Disable();
        return;
      }
    ...
    public bool GetActive() {
      return graphic.GetText() != string.Empty || graphic.GetTextScale() < 0.1 f;
    }
